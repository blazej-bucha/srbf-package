...
%==========================================================================
%
%The source code of GrafLab and the pdf file 
...
%
%==========================================================================
%LIST OF CHANGES IN GrafLab (GRAvity Field LABoratory)
%==========================================================================
%
%See the file "list_of_changes_in_GrafLab.txt".
%
%
%
%==========================================================================
%USER MANUAL (see also the "user_manual_GrafLab.txt" file)
%==========================================================================
%
...
%TEST RUN:
%==========================================================================
%To obtain the same output as in the enclosed sample output file 
%("output_EGM96.txt"), please use the following command (the input files 
%"EGM96.mat" and "input.txt" are expected to be stored in the same folder
%as the "GrafLab.m" file):
%
...
if nargin==0  
    ...
    %Main window 
    M=figure('units','pixels','numbertitle','off','name','GrafLab 2.1.2',...
    ...
    switch(vstpar)
        ...
        case('OK') %Click on the OK button in the main GrafLab window 
            ...
            if STD==0
                ...
                if volbagridcheck==1
                    ...
                    if GUI==0 && strcmp(fistep,'empty') && strcmp(fimax,'empty')
                        ...
                        if fimin>90 || fimin<-90
                            if GUI==1
                                errordlg('The value of "Lat. min" must be within the interval <-90�,90�>.',...
                                    'Error in irregular surface selection panel');
                                error('The value of "Lat. min" must be within the interval <-90�,90�>.');
                            elseif GUI==0
                                error('The value of "lat_min" must be within the interval <-90�,90�>.');
                            end
                        end
                        if fimax>90 || fimax<-90
                            if GUI==1
                                errordlg('The value of "Lat. max" must be within the interval <-90�,90�>.',...
                                    'Error in irregular surface selection panel');
                                error('The value of "Lat. max" must be within the interval <-90�,90�>.');
                            elseif GUI==0
                                error('The value of "lat_max" must be within the interval <-90�,90�>.');
                            end
                        end
                    ...
                    if GUI==0 && strcmp(lambdastep,'empty') && strcmp(lambdamax,'empty')
                        ...
                        if lambdamin>360 || lambdamin<-180
                            if GUI==1
                                errordlg('The value of "Lon. min" must be within the interval <-180�,180�> or <0�,360�>.',...
                                    'Error in irregular surface selection panel');
                                error('The value of "Lon. min" must be within the interval <-180�,180�> or <0�,360�>.');
                            elseif GUI==0
                                error('The value of "lon_min" must be within the interval <-180�,180�> or <0�,360�>.');
                            end
                        end
                        if lambdamax>360 || lambdamax<-180
                            if GUI==1
                                errordlg('The value of "Lon. max" must be within the interval <-180�,180�> or <0�,360�>.',...
                                    'Error in irregular surface selection panel');
                                error('The value of "Lon. max" must be within the interval <-180�,180�> or <0�,360�>.');
                            elseif GUI==0
                                error('The value of "lon_max" must be within the interval <-180�,180�> or <0�,360�>.');
                            end
                        end
                        if (lambdamax-lambdamin)>360
                            if GUI==1
                                errordlg('The longitudes must be in the range <-180�,180�> or <0�,360�>.',...
                                    'Error in irregular surface selection panel');
                                error('The longitudes must be in the range <-180�,180�> or <0�,360�>.');
                            elseif GUI==0
                                error('The longitudes must be in the range <-180�,180�> or <0�,360�>.');
                            end
                        end
                ...
                if volbadiskcheck==1                             
                    ...
                    if any(fi>90) || any(fi<-90)
                        if GUI==1 %If working with the GUI
                            errordlg('The values of latitude must be within the interval <-90�,90�>.',...
                                'Error in point type selection');
                            error('The values of latitude must be within the interval <-90�,90�>.');
                        elseif GUI==0
                            error('The values of the "lat" variable must be within the interval <-90�,90�>.');
                        end
                    end
                    if any(lambda>360) || any(lambda<-180)
                        if GUI==1 %If working with the GUI
                            errordlg('The values of longitudes must be within the interval <-180�,180�> or <0�,360�>.',...
                                'Error in point type selection');
                            error('The values of longitudes must be within the interval <-180�,180�> or <0�,360�>.');
                        elseif GUI==0
                            error('The values of the "lon" variable must be within the interval <-180�,180�> or <0�,360�>.');
                        end
                    end
            ...
            if Export_report==1
                ...
                if GUI==1 %Working with the GUI
                    fprintf(exp,'Software                                           \tGrafLab 2.1.2 (GUI used)\n');
                elseif GUI==0 %Working without the GUI
                    fprintf(exp,'Software                                           \tGrafLab 2.1.2 (GUI not used)\n');
                end
